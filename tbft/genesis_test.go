...
func TestOutGenesisJson(t *testing.T) {
	...
	b, _ := ioutil.ReadFile("E:\\src\\github.com\\truechain\\discovery\\cmd\\discovery\\genesis.json")
	...
	for i := 1; i <= 52; i++ {
		...
		e := ioutil.WriteFile("E:\\src\\github.com\\truechain\\discovery\\cmd\\discovery\\"+fmt.Sprintf("genesis_%s.json", strconv.Itoa(i)), bytes, os.ModePerm)
		if e != nil {
